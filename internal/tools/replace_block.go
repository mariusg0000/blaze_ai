...
func (t *ReplaceBlockTool) Description() string {
	return "file_path + blocks → replace every uniquely matching exact block in one file; prefer blocks for multiple edits; failed blocks are returned verbatim with the live file content when under 500000 bytes"
}
...
func (t *ReplaceBlockTool) Parameters() json.RawMessage {
	return json.RawMessage(`{
		...
		"properties": {
			...
			"blocks": {
				"type": "array",
				"description": "Prefer blocks when making two or more edits in the same file. Each old_block must match exactly once, including whitespace and newlines. If a block fails, correct and retry only that block; do not resend successful blocks.",
				"items": {
					"type": "object",
					"properties": {
...
func failedBlockReport(path, content string, failures []string) string {
	...
	if len([]byte(content)) < replaceBlockFileContentLimit {
		builder.WriteString("\nCurrent file content (live from disk):\n--- FILE START ---\n")
		builder.WriteString(content)
		if !strings.HasSuffix(content, "\n") {
			builder.WriteByte('\n')
		}
		builder.WriteString("--- FILE END ---\nRetry only the failed blocks with corrected exact text; do not resend successful blocks.")
	} else {
		builder.WriteString(fmt.Sprintf("\nCurrent file content omitted: file is %d bytes and exceeds the 500000-byte limit. Retry only the failed blocks after inspecting the relevant content.", len([]byte(content))))
	}
