...
func (h *Hint) renderLocked() (text string) {
	...
	// then completion/command hints. Do not append a newline after the final
	// lane so a persistent footer does not leave an empty row below it.
	lanes := [][]rune{h.persistent, h.provided, h.transient, h.text}
	last := -1
