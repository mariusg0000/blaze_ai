...
func (e *Engine) renderHelpers() {
	...
	// The final hint lane no longer emits a trailing newline, so the cursor
	// is on the final hint row rather than on the row below it. Move over
	// the preceding hint rows, then move above the final hint row.
